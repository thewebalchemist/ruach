...
import {
  LayoutDashboard, Users, Calendar, BookOpen, Bell,
  Menu, X, Sun, Moon, LogOut, Settings, ClipboardList, Package, Loader2, ShieldOff, ChevronRight,
} from 'lucide-react';
...
export function CrosspointLayout({ children, crosspoint, title }: CrosspointLayoutProps) {
  ...
  // This is the leader portal for one specific crosspoint — closes the
  // "0 of 10 leader-facing pages have an auth gate" finding. A caller must
  // either be a global admin/pastor or hold a leader/assistant/treasurer
  // membership for THIS crosspoint (matches the check already done in
  // pages/crosspoint/index.tsx's login redirect).
  useEffect(() => {
    ...
    supabase
      ...
      .eq('crosspoint_id', crosspoint.id)
  ...
  }, [authLoading, session?.user?.id, isAdmin, crosspoint.id]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F0F2F5] dark:bg-[#080808] p-6">
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm text-gray-500 mb-6">You're not a leader of this crosspoint.</p>
          <button onClick={() => router.push('/crosspoint')} className="btn btn-primary">Sign in</button>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between px-4 pt-5 pb-4">
          <Link href="/crosspoint" className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-black text-gray-900 dark:text-white leading-tight tracking-tight truncate">{crosspoint.name}</p>
              <p className="text-[10px] text-gray-400 leading-tight truncate">{crosspoint.area}</p>
            </div>
          </Link>
          <button
            onClick={() => setSidebarOpen(false)}
          ...
          ">
            ...
            <div className="flex items-center gap-1">
              ...
              <button className="relative p-2.5 rounded-xl text-gray-500 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-700 dark:hover:text-white transition-colors">
                ...
                <span className="absolute top-2 right-2 w-2 h-2 bg-[#BF0A30] rounded-full ring-2 ring-white dark:ring-[#111]" />
              </button>
            </div>
          </div>
        </header>
