...
const STATUS_COLORS: Record<string, string> = {
  ...
  'completed':         'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-white/50',
  ...
  return (
    <ConnectLayout title="Cohorts">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Cohorts</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 dark:border-white/[0.06] rounded-xl bg-white dark:bg-[#12151C] text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30]"
          />
        </div>
      ...
      <div className="space-y-3">
        {filtered.map(cohort => {
          ...
          return (
            <div key={cohort.id} className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 shadow-sm overflow-hidden">
              <div className="p-5">
                {/* Title row */}
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h2 className="font-semibold text-gray-900 dark:text-white truncate">{cohort.name}</h2>
                      <span className={`px-2 py-0.5 text-xs font-semibold rounded-full flex-shrink-0 capitalize ${STATUS_COLORS[cohort.status] ?? 'bg-gray-100 text-gray-700 dark:bg-white/5'}`}>
                  ...
                  <Link
                    ...
                    className="flex-shrink-0 flex items-center gap-1 px-3 py-1.5 border border-gray-200 dark:border-white/[0.06] rounded-xl text-xs font-medium text-gray-600 dark:text-white/50 hover:border-[#BF0A30] hover:text-[#BF0A30] transition-colors"
                  >
                ...
                {/* Footer */}
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100 dark:border-white/[0.04]">
        ...
        {filtered.length === 0 && (
          <div className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 p-16 text-center">
