...
export default function CohortDetailPage() {
  ...
  const STATUS_COLOR: Record<string, string> = {
    ...
    completed:          'bg-gray-100 text-gray-700 dark:bg-white/5',
    ...
    <ConnectLayout title={cohort.name}>
      ...
      {showResource && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200 dark:border-white/[0.06] p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              {/* Session picker */}
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Attach to Session *</label>
                <div className="relative">
                  <select
                    value={resSessionId}
                    onChange={e => setResSessionId(e.target.value)}
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-gray-50 dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white appearance-none focus:outline-none focus:border-[#BF0A30]"
                  >
                    {sessions.map(s => (
              ...
              {/* Type buttons */}
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Resource Type</label>
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border-2 text-xs font-medium transition-colors ${
                          resType === key
                            ...
                            : 'border-gray-200 dark:border-white/[0.06] text-gray-500 hover:border-gray-300'
                        }`}
                      >
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setShowResource(false)} className="flex-1 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm font-medium text-gray-600 dark:text-white/50">
                Cancel
              </button>
              <button
                onClick={addResource}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200 dark:border-white/[0.06] p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-gray-900 dark:text-white">Notify All Students</h3>
              <button onClick={() => setShowNotify(false)} className="p-1.5 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400">
                <X className="w-4 h-4" />
              </button>
            </div>
            <textarea
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowNotify(false)} className="flex-1 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm font-medium text-gray-600 dark:text-white/50">Cancel</button>
              <button
      ...
      {/* ── Header ── */}
      <Link href="/connect/cohorts" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 mb-5 transition-colors">
        <ArrowLeft className="w-4 h-4" /> All Cohorts
      </Link>

      <div className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 p-5 mb-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => openResourceModal()} className="flex items-center gap-1.5 px-3 py-2 border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm text-gray-600 dark:text-white/50 hover:border-[#BF0A30] hover:text-[#BF0A30] transition-colors">
      ...
      {/* ── Tabs ── */}
      <div className="flex gap-1 bg-white dark:bg-[#12151C] border border-gray-200 dark:border-white/[0.06]/70 p-1 rounded-2xl mb-5 shadow-sm">
        {([
                  ...
                  {students.map(s => {
                    ...
                    return (
                      <tr key={s.id} className="border-b border-gray-50 dark:border-white/[0.02] hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className="w-12 h-1.5 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
      ...
      {activeTab === 'exams' && (
        <div className="space-y-3">
          ...
          {exams.length === 0 ? (
            ...
            <div key={exam.id} className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 p-5 shadow-sm">
      ...
      {activeTab === 'resources' && (
        <div className="space-y-4">
          ...
          {allResources.length === 0 ? (
            <div className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 p-12 text-center shadow-sm">
