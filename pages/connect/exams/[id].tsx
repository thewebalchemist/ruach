...
import {
  ...
  BookOpen, TrendingUp, Send, Eye, Pencil, ChevronRight
} from 'lucide-react';
import { ConnectLayout } from '@/components/connect/ConnectLayout';
import { mockConnectExams, mockConnectStudents, mockConnectCohorts, getUserById } from '@/data/connect';
import { ConnectExam, ExamStatus } from '@/types';

export default function ExamDetailPage() {
  const router = useRouter();
  const { id } = router.query as { id: string };

  const [exam, setExam] = useState<ConnectExam | undefined>(
    mockConnectExams.find(e => e.id === id)
  );
  ...
  const students = mockConnectStudents.filter(s => s.cohortId === exam.cohortId);

  // Students who have a result for this exam
  const results = students.flatMap(s =>
    s.examResults
      .filter(r => r.examId === exam.id)
      .map(r => ({ ...r, student: s, user: getUserById(s.userId) }))
  );
  ...
  const toggleStatus = (status: ExamStatus) => {
    // In production: PATCH /api/connect/exams/:id
    ...
    <ConnectLayout title={exam.title}>
      ...
      <div className="bg-[#12151C] rounded-2xl border border-white/[0.06]/70 p-5 mb-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {cohort?.name} · {exam.questions.length} questions · {exam.durationMinutes} min · Pass: {exam.passingMarks}/{exam.totalMarks}
            </p>
            {exam.description && (
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {exam.status === 'draft' && (
              <button onClick={() => toggleStatus('published')} className="flex items-center gap-1.5 px-3 py-2 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-700 transition-colors">
                <Send className="w-4 h-4" /> Publish
              </button>
            )}
            {exam.status === 'published' && (
              <button onClick={() => toggleStatus('closed')} className="flex items-center gap-1.5 px-3 py-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-xl text-sm font-medium hover:bg-red-200 transition-colors">
                <Clock className="w-4 h-4" /> Close Exam
              </button>
            )}
            {exam.status === 'closed' && (
              <button onClick={() => toggleStatus('published')} className="flex items-center gap-1.5 px-3 py-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-xl text-sm font-medium hover:bg-blue-200 transition-colors">
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-xs text-gray-500">
          ...
          <span>·</span>
          <span>Closes: {new Date(exam.availableUntil).toLocaleString()}</span>
        </div>
      </div>
                  ...
                  <tbody>
                    {results.map(r => (
                      <tr key={r.examId + r.student.id} className="border-b border-gray-50 dark:border-white/[0.02] hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-xs text-gray-400">{new Date(r.submittedAt).toLocaleDateString()}</td>
                        <td className="px-4 py-3">
                          <Link href={`/connect/students/${r.student.id}`} className="p-1.5 text-gray-400 hover:text-blue-500 rounded-lg inline-block">
