    ...
    <ConnectLayout title="Create Exam">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2">
          <button onClick={() => handleSave('draft')} disabled={!title.trim() || saving}
            className="flex items-center gap-1.5 px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm font-medium text-gray-600 dark:text-white/50 disabled:opacity-40 hover:border-gray-300 dark:hover:border-white/10 transition-colors">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          {questions.map((q, idx) => (
            <div key={q.id} className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 shadow-sm overflow-hidden">

              {/* Header */}
              <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-100 dark:border-white/[0.04] bg-gray-50 dark:bg-white/[0.02]">
                ...
                <p className="flex-1 text-sm font-medium text-gray-700 dark:text-white/70 truncate">
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex gap-3">
                    <div className="flex-1">
                      <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Type</label>
                      <div className="flex gap-2">
                    ...
                    <div className="w-24">
                      <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Marks</label>
                      <input type="number" min={1} max={20} value={q.marks}
                        onChange={e => updateQ(q.id, { marks: Math.max(1, parseInt(e.target.value) || 1) })}
                        className="w-full px-3 py-2 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-gray-50 dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white text-center focus:outline-none focus:border-[#BF0A30]"
                      />
                    </div>
                  </div>
                  ...
                  {/* Question text */}
                  <div>
                    <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Question *</label>
                    <textarea value={q.question} onChange={e => updateQ(q.id, { question: e.target.value })}
                      rows={2} placeholder="Type the question..."
                      className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-gray-50 dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white resize-none focus:outline-none focus:border-[#BF0A30] placeholder:text-gray-400"
                    />
                  </div>
                        ...
                        return (
                          <div key={i} className="flex items-center gap-2">
                            ...
                            {q.questionType === 'true-false' ? (
                              <span className="flex-1 px-3 py-2 bg-gray-50 dark:bg-[#0A0C10] border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm text-gray-500 dark:text-white/50">
                                {opt}
                              </span>
                            ) : (
                              <input value={opt} onChange={e => setOption(q.id, i, e.target.value)}
                                placeholder={`Option ${String.fromCharCode(65 + i)}`}
                                className="flex-1 px-3 py-2 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-gray-50 dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30] placeholder:text-gray-400"
                              />
                            )}
                          </div>
                        );
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-4" style={H}>Exam Details</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Title *</label>
                <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Session 2 Quiz"
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-gray-50 dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30] placeholder:text-gray-400"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Description</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Cohort *</label>
                {loadingCohorts ? (
                  <div className="h-10 rounded-xl bg-gray-100 dark:bg-white/5 animate-pulse" />
                ) : (
                  <select value={cohortId} onChange={e => { setCohortId(e.target.value); setSessionId(''); }}
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-gray-50 dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30]">
              ...
              {sessions.length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Linked Session (optional)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Duration (minutes)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Pass Requirement (%)</label>
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm font-bold text-gray-700 dark:text-white/70 w-10 text-right">{passingPct}%</span>
                </div>
          ...
          <div className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 p-5 shadow-sm">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Opens</label>
                <input type="datetime-local" value={availableFrom} onChange={e => setAvailableFrom(e.target.value)}
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-gray-50 dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30]"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 dark:text-white/50 mb-1.5">Closes</label>
          ...
          <div className="bg-gray-50 dark:bg-[#0A0C10] rounded-2xl border border-gray-200/70 dark:border-white/[0.06]/70 p-4">
            <div className="flex items-center gap-2 mb-3">
              <BookOpen className="w-4 h-4 text-gray-500" />
              <p className="text-sm font-semibold text-gray-700 dark:text-white/70" style={H}>Summary</p>
            </div>
            <div className="space-y-1.5 text-xs text-gray-500">
              {[
                ...
                <div key={String(k)} className="flex justify-between">
                  <span>{k}</span>
                  <span className="font-medium text-gray-700 dark:text-white/70">{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <button onClick={() => handleSave('draft')} disabled={!title.trim() || saving}
              className="w-full py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm font-medium text-gray-600 dark:text-white/50 disabled:opacity-40 hover:border-gray-300 dark:hover:border-white/10 transition-colors flex items-center justify-center gap-2">
