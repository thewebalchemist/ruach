  ...
  return (
    <AdminLayout title="Create Notice">
      <div className="max-w-3xl">
        <Link href="/admin/notices" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 dark:hover:text-white/70 mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-200 dark:border-[#2D2D2D] p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white/70 mb-1">Title *</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white/70 mb-1">Content *</label>
          ...
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-200 dark:border-[#2D2D2D] p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white/70 mb-1">Scope *</label>
              ...
              {formData.scope === 'department' && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-white/70 mb-1">Department</label>
          ...
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-200 dark:border-[#2D2D2D] p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white/70 mb-1">Priority</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-white/70 mb-1">Expires On</label>
          ...
          {formData.title && (
            <div className="bg-gray-50 dark:bg-white/[0.04] rounded-xl p-6">
              <h3 className="text-sm font-medium text-gray-500 mb-3">Preview</h3>
              <div className={`bg-white dark:bg-[#12151C] rounded-lg p-4 border-l-4 ${formData.priority === 'high' ? 'border-l-red-500' : formData.priority === 'medium' ? 'border-l-amber-500' : 'border-l-gray-300'}`}>
                <div className="flex items-center gap-2 mb-2">
                  <Bell className="w-4 h-4 text-[#BF0A30]" />
                  <span className="font-semibold text-gray-900 dark:text-white">{formData.title}</span>
                </div>
                <p className="text-sm text-gray-600 dark:text-white/50 whitespace-pre-wrap">{formData.content}</p>
              </div>
            </div>
          )}
