...
interface CrosspointMembership {
  role: string;
  crosspoints: { id: string; name: string; area: string; meeting_day: string | null; meeting_time: string | null } | null;
}

interface DepartmentMembership {
  role: string;
  joined_date: string;
  departments: { id: string; name: string; icon_name: string | null } | null;
}

interface DiscStudent {
  level: number;
  status: string;
  enrolled_at: string;
...
export default function MemberDetailPage() {
  ...
  const [cpMembership, setCpMembership] = useState<CrosspointMembership | null>(null);
  const [deptMemberships, setDeptMemberships] = useState<DepartmentMembership[]>([]);
  const [discStudents, setDiscStudents] = useState<DiscStudent[]>([]);
  ...
  async function loadMember() {
    ...
    const [
      ...
      db.from('crosspoint_memberships').select('role, crosspoints(id, name, area, meeting_day, meeting_time)')
        .eq('user_id', id).eq('status', 'active').limit(1).maybeSingle(),
      db.from('department_memberships').select('role, joined_date, departments(id, name, icon_name)')
        .eq('user_id', id).eq('status', 'active'),
      db.from('discipleship_students').select('level, status, enrolled_at, completed_at')
        .eq('user_id', id)
        .order('level', { ascending: true }),
    ]);

    setMember(profileData as MemberProfile | null);
    setCpMembership(cpData as CrosspointMembership | null);
    setDeptMemberships((deptData ?? []) as DepartmentMembership[]);
    setDiscStudents((discData ?? []) as DiscStudent[]);
    setLoading(false);
  }
  ...
  const crosspoint = cpMembership?.crosspoints ?? null;
  const currentDisc = discStudents.find(s => ['enrolled', 'in-progress'].includes(s.status));
  const completedLevels = discStudents.filter(s => ['graduated', 'completed'].includes(s.status));
          ...
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-200 dark:border-[#2D2D2D] p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4 text-sm">
              ...
              {member.date_of_birth && (
                <div><span className="text-gray-500">Date of Birth:</span> <span className="font-medium ml-1">{member.date_of_birth}</span></div>
              )}
              {member.occupation && (
                <div><span className="text-gray-500">Occupation:</span> <span className="font-medium ml-1">{member.occupation}</span></div>
              )}
              {member.marital_status && (
                <div><span className="text-gray-500">Marital Status:</span> <span className="font-medium capitalize ml-1">{member.marital_status}</span></div>
              )}
              ...
              <div><span className="text-gray-500">Zone:</span> <span className="font-medium capitalize ml-1">{member.crosspoint_zone ?? '—'}</span></div>
            </div>
          </div>
            ...
            {member.connect_graduated_at ? (
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Completed {new Date(member.connect_graduated_at).toLocaleDateString()}
                </p>
                ...
                {currentDisc && (
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Since {new Date(currentDisc.enrolled_at).toLocaleDateString()}
                    </p>
                  </div>
                )}
                        ...
                        <div key={cl.level} className="flex items-center justify-between text-sm">
                          ...
                          {cl.completed_at && (
                            <span className="text-xs text-gray-400">
                              {new Date(cl.completed_at).toLocaleDateString()}
                            </span>
                          )}
                        </div>
                      ))}
