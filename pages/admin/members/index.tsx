...
export default function MembersPage() {
  ...
  const filteredMembers = mockMembers.filter(member => {
    const matchesSearch = `${member.firstName} ${member.lastName} ${member.memberId} ${member.phone}`.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <AdminLayout title="Members">
      ...
      <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-200 dark:border-[#2D2D2D] overflow-hidden">
                ...
                <tr key={member.id} className="hover:bg-gray-50 dark:hover:bg-[#252525]">
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-[#BF0A30] flex items-center justify-center text-white text-sm font-semibold">{member.firstName[0]}{member.lastName[0]}</div>
                      <div>
                  ...
                  <td className="py-3 px-4 font-mono text-sm">{member.memberId}</td>
                  <td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">{member.crosspoint?.crosspointName || '—'}</td>
        ...
        {filteredMembers.length === 0 && <div className="p-12 text-center text-gray-500">No members found</div>}
      </div>
