...
type SubmitResponse =
  | { success: true; resultId: string; score: number; totalMarks: number; percentage: number; passed: boolean; passingMarks: number; achievements: string[]; }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<SubmitResponse>) {
  ...
  if (!examId || !studentId || !answers) return res.status(400).json({ success: false, error: 'Missing required fields' });

  // 1. Load exam
  const { data: exam, error: examErr } = await (supabaseAdmin as any)
  ...
  // 2. Verify student
  const { data: student, error: studentErr } = await (supabaseAdmin as any)
  ...
  const { data: existing } = await (supabaseAdmin as any)
    .from('discipleship_exam_results').select('id').eq('exam_id', examId).eq('student_id', studentId).maybeSingle();
  ...
  const { data: questions, error: qErr } = await (supabaseAdmin as any)
    .from('discipleship_exam_questions')
  ...
  // 6. Write result
  const { data: result, error: resultErr } = await (supabaseAdmin as any)
  ...
  const answerRows = Object.entries(answers).map(([questionId, sel]) => ({
    ...
    selected_answer:  sel[0] ?? -1,
    selected_answers: sel,
  }));
  if (answerRows.length > 0) {
    await (supabaseAdmin as any).from('discipleship_exam_answers').insert(answerRows).then(({ error }: { error: any }) => {
  ...
  await (supabaseAdmin as any).from('notifications').insert({
    ...
    action_url: '/discipleship/student',
  });

  // 9. Check first-ever submission
  const { count: prevCount } = await (supabaseAdmin as any)
    .from('discipleship_exam_results')
