...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    const { data, error } = await supabaseAdmin.from('sermons').insert(req.body).select().single();
  ...
  if (req.method === 'PUT') {
    ...
    const { data, error } = await supabaseAdmin.from('sermons').update(fields).eq('id', id).select().single();
    if (error) return res.status(400).json({ error: error.message });
    return res.status(200).json(data);
  }
