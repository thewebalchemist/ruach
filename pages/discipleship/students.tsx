...
import { Search, AlertTriangle, CheckCircle, BookOpen } from 'lucide-react';
...
type StudentStatus = 'in-progress' | 'completed' | 'failed' | 'withdrawn';
...
export default function DiscipleshipStudentsPage() {
  ...
  const enriched = mockDiscipleshipStudents.map(s => {
  ...
  const filtered = enriched.filter(s => {
    ...
    const matchesRisk = !filterAtRisk || s.totalAttendancePercent < 80;
    return matchesSearch && matchesLevel && matchesStatus && matchesRisk;
  });

  const stats = {
    total: enriched.length,
    inProgress: enriched.filter(s => s.status === 'in-progress').length,
    atRisk: enriched.filter(s => s.totalAttendancePercent < 80).length,
                  ...
                  <tr key={student.id} className={`hover:bg-gray-50 dark:hover:bg-[#252525] ${isAtRisk ? 'border-l-2 border-amber-400' : ''}`}>
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-sm font-medium ${student.totalAttendancePercent >= 80 ? 'text-green-600' : 'text-amber-600'}`}>
                    ...
                    <td className="py-3 px-4">
                      {student.averageExamScore > 0 ? (
                        <span className={`text-sm font-medium ${student.averageExamScore >= 70 ? 'text-green-600' : 'text-red-600'}`}>
