    ...
    <DiscipleshipLayout title="Create Exam">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2">
          <button onClick={() => handleSave('draft')} disabled={!title.trim() || saving}
            className="flex items-center gap-1.5 px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm font-medium text-gray-500 dark:text-white/50 disabled:opacity-40 hover:border-gray-300 dark:hover:border-white/10 transition-colors">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          {questions.map((q, idx) => (
            <div key={q.id} className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200 dark:border-white/[0.06]/70 shadow-sm overflow-hidden">
                ...
                <div className="p-5 space-y-4">
                  <div className="flex gap-3">
                    <div className="flex-1">
                      <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Type</label>
                      <div className="flex gap-2">
                    ...
                    <div className="w-24">
                      <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Marks</label>
                      <input type="number" min={1} max={20} value={q.marks}
                        onChange={e => updateQ(q.id, { marks: Math.max(1, parseInt(e.target.value) || 1) })}
                        className="w-full px-3 py-2 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-white dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white text-center focus:outline-none focus:border-[#BF0A30]"
                      />
                    </div>
                  </div>

                  <div>
                    <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Question *</label>
                    <textarea value={q.question} onChange={e => updateQ(q.id, { question: e.target.value })}
                      rows={2} placeholder="Type the question..."
                      className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-white dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white resize-none focus:outline-none focus:border-[#BF0A30] placeholder:text-gray-400"
                    />
                  </div>
                        ...
                        return (
                          <div key={i} className="flex items-center gap-2">
                            ...
                            {q.questionType === 'true-false' ? (
                              <span className="flex-1 px-3 py-2 bg-white dark:bg-[#0A0C10] border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm text-gray-500 dark:text-white/50">
                                {opt}
                              </span>
                            ) : (
                              <input value={opt} onChange={e => setOption(q.id, i, e.target.value)}
                                placeholder={`Option ${String.fromCharCode(65 + i)}`}
                                className="flex-1 px-3 py-2 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-white dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30] placeholder:text-gray-400"
                              />
                            )}
                          </div>
                        );
          ...
          <button onClick={addQ}
            className="flex items-center justify-center gap-2 w-full py-3.5 border-2 border-dashed border-gray-200 dark:border-[#2D2D2D] rounded-2xl text-sm text-gray-500 hover:border-[#BF0A30] hover:text-[#BF0A30] transition-colors">
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200 dark:border-white/[0.06]/70 p-5 shadow-sm">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. KDC Session 2 Quiz"
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-white dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30] placeholder:text-gray-400"
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Cohort *</label>
                {loadingCohorts ? (
                  ...
                  <select value={cohortId} onChange={e => { setCohortId(e.target.value); setSessionId(''); }}
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-white dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30]">
              ...
              {sessions.length > 0 && (
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Linked Session (optional)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Duration (minutes)</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Pass Requirement (%)</label>
          ...
          <div className="bg-white dark:bg-[#12151C] rounded-2xl border border-gray-200 dark:border-white/[0.06]/70 p-5 shadow-sm">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Opens</label>
                <input type="datetime-local" value={availableFrom} onChange={e => setAvailableFrom(e.target.value)}
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl bg-white dark:bg-[#0A0C10] text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#BF0A30]"
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-white/50 mb-1.5">Closes</label>
          ...
          <div className="bg-gray-50 dark:bg-[#0A0C10] rounded-2xl border border-gray-200 dark:border-white/[0.06]/70 p-4">
          ...
          <div className="space-y-2">
            ...
            <button onClick={() => handleSave('draft')} disabled={!title.trim() || saving}
              className="w-full py-2.5 border border-gray-200 dark:border-white/[0.06] rounded-xl text-sm font-medium text-gray-500 dark:text-white/50 disabled:opacity-40 hover:border-gray-300 dark:hover:border-white/10 transition-colors flex items-center justify-center gap-2">
